...
function mode() {
  alert("currently other mode are not available. but i can tell you some thing about the current mode -- This is a simple game where you control a block that moves around the canvas. The goal is to collect red dots while avoiding collisions with the trail of your own block. Use the arrow keys or swipe gestures to change direction. The game ends if you hit the edges of the canvas or collide with your own trail.");
};
...
function update() {
  let scoreValue = trainBox - 3;
  score.innerText = trainBox - 3; // Initial score
  if (scoreValue > score2.innerText) {
  score2.innerText = scoreValue; // Update score2 if scoreValue is greater
  };
  // Move player
  playerX += dx;
  playerY += dy;
...
document.addEventListener("keydown", function (event) {
  if (!isPlaying) return; // Ignore key presses if not playing
